    ...
    const fetchUsers = async () => {
      try {
        ...
        if (data.length === 0) {
          ...
          const goodsReceiptCreator = data.filter(
            (user) => user.permissions.goodsReceiptCreator
          );

          setUsers(goodsReceiptCreator);
        }
      } catch (error) {
  ...
  return (
    <div>
      ...
      <div className="container mx-auto p-4 bg-white">
        <h1 className="text-3xl font-bold mb-4 text-center">Goods Receipt Creator</h1>
        {loading ? (
...
export default GoodsReceiptCreator;
