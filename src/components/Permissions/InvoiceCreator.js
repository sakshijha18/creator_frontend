    ...
    const fetchUsers = async () => {
      try {
        ...
        if (data.length === 0) {
          ...
          const invocieCreator = data.filter(
            (user) => user.permissions.invoiceCreator
          );

          setUsers(invocieCreator);
        }
      } catch (error) {
  ...
  return (
    <div>
      ...
      <div className="container mx-auto p-4 bg-white">
        <h1 className="text-3xl font-bold mb-4 text-center">Invoice Creator</h1>
        {loading ? (
