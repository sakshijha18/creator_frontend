    ...
    const fetchUsers = async () => {
      try {
        ...
        if (data.length === 0) {
          ...
          const vendorApprover = data.filter(
            (user) => user.permissions.vendorApprover
          );

          setUsers(vendorApprover);
        }
      } catch (error) {
  ...
  return (
    <div>
      ...
      <div className="container mx-auto p-4 bg-white">
        <h1 className="text-3xl font-bold mb-4 text-center">Vendor Approver</h1>
        {loading ? (
